...
export function QuickChat({ sessionId: externalSessionId, onSessionChange, className = "" }: QuickChatProps) {
  ...
  // 🔄 Sync prop -> state: when sidebar selects a different session
  useEffect(() => {
  ...
  const sendMessage = async (content: string, _files?: any) => {
    ...
    const userMsg: ChatMessage = {
      id: uuidv4(), // ✅ FIXED: replaced crypto.randomUUID()
      content,
      sender: 'user',
    ...
    <div className={`flex flex-col h-full ${className}`}>
      {showEmptyState ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-6">
          <div className="text-center text-2xl font-semibold text-gray-300 select-none">
